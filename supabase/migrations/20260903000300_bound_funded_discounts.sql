-- Bounds a funded discount at the point the money moves. Letting a settled amount
-- sit below the list total (20260901000300) removed the ordering check and left
-- nothing in its place, so a caller could send any final amount it liked and this
-- function would book it. The bound belongs here because this is the only place
-- that holds both halves of the answer: what the goods cost the merchant, and
-- what discount this buyer has already earned.
--
-- A premium above list stays unbounded, which is what an uplift is. A discount
-- below list may reach the list total less the entitlement the buyer qualifies
-- for, and never below cost.
--
-- This also restores the privileges the function lost along the way.
-- 20260830000200 dropped the eight argument version to add p_run_id, and the
-- revoke and grant written in 20260823000100 named that signature, so they went
-- with it. The nine argument version has been running on the default execute to
-- public since then.
...
begin
  ...
  values (p_account_id, order_row.id, 'gate', 'gate_approve', 'wallet balance, catalog, stock, discount entitlement, and duplicate checks passed', jsonb_build_object('base_amount_paise', p_base_amount_paise, 'final_amount_paise', p_final_amount_paise, 'razorpay_order_id', p_razorpay_order_id), p_run_id);
