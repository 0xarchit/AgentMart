-- Carry the run a purchase belongs to onto the trail rows written inside the
-- fulfilment transaction, so money rows trace back to the conversation that
-- produced them. The parameter defaults to null so an older caller still works.
...
drop function if exists public.fulfill_wallet_order(uuid, uuid, integer, bigint, bigint, text, text, integer);
