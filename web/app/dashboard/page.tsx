...
export default async function DashboardPage() {
  ...
  const [accountResult, ordersResult, ledgerResult, revenueResult, auditResult] = await Promise.all([
    ...
    supabase.from("audit_log").select("id,order_id,actor,action,reason,created_at").eq("account_id", user.id).order("created_at", { ascending: false }).limit(100),
  ]);
