...
export function AuditTimeline({ events }: AuditTimelineProps) {
  ...
  const visibleEvents = useMemo(() => {
    ...
    return events.filter((event) => {
      ...
      const matchesOrder =
        query === "" || event.order_id?.toLowerCase().includes(query);
      ...
      <div className="flex flex-col gap-4 border-b border-ink/10 pb-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-ink/60">
            Account-scoped decisions, wallet movements, and fulfillment
            outcomes.
          </p>
        </div>
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
          <label className="flex flex-col gap-1 text-xs font-semibold uppercase tracking-[0.12em] text-ink/60">
            ...
            <select
              ...
              className="min-w-44 border border-ink/20 bg-white px-3 py-2 text-sm font-normal normal-case tracking-normal text-ink"
            >
          ...
          <label className="flex flex-col gap-1 text-xs font-semibold uppercase tracking-[0.12em] text-ink/60">
            ...
            <input
              ...
              className="border border-ink/20 px-3 py-2 text-sm font-normal normal-case tracking-normal text-ink"
            />
          </label>
          <AuditRefresh />
        </div>
      ...
      <div className="divide-y divide-ink/10">
        {visibleEvents.length === 0 ? (
          ...
          visibleEvents.map((event) => (
            <article
              key={event.id}
              className="grid gap-2 py-4 text-sm md:grid-cols-[9rem_1fr_auto] md:items-start"
            >
              ...
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  <span
                    className={`px-2 py-1 text-xs font-semibold ${actionTone(event.action)}`}
                  >
                    {event.action}
                  </span>
                  <span className="text-xs text-ink/70">by {event.actor}</span>
                </div>
              ...
              <p className="font-mono text-xs text-ink/70">
                {event.order_id ? event.order_id.slice(0, 8) : "No order"}
              </p>
            </article>
          ))
        )}
