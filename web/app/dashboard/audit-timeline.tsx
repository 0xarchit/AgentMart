  ...
  return (
    <section className="border border-ink/10 bg-white p-5 lg:col-span-2">
      ...
      <div className="divide-y divide-ink/10">
        {visibleEvents.length === 0 ? <p className="py-6 text-sm text-ink/70">No audit events match these filters.</p> : visibleEvents.map((event) => (
          <article key={event.id} className="grid gap-2 py-4 text-sm md:grid-cols-[9rem_1fr_auto] md:items-start">
            <p className="text-xs text-ink/70">{formatTimestamp(event.created_at)}</p>
            <div>
            ...
            <p className="font-mono text-xs text-ink/70">{event.order_id ? event.order_id.slice(0, 8) : "No order"}</p>
          </article>
        ))}
      </div>
    </section>
