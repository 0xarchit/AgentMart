...
export default async function AdminPage() {
  ...
  const [revenueResult, ordersResult, auditResult] = await Promise.all([
    ...
    admin
      .from("orders")
      .select("id,account_id,amount_paise,status,created_at")
      .order("created_at", { ascending: false })
      .limit(100),
    admin
      .from("audit_log")
      .select("id,account_id,actor,action,reason,created_at")
      .order("created_at", { ascending: false })
      .limit(100),
  ]);
      ...
      <div className="mx-auto max-w-6xl">
        <nav className="flex items-center justify-between border-b border-ink/10 pb-5">
          ...
          <div className="flex gap-4 text-sm">
            ...
            <Link href="/admin" className="font-semibold text-moss">
              Admin
            </Link>
          </div>
        </nav>
        <div className="mt-8">
          ...
          <h1 className="mt-2 text-4xl font-semibold">Admin revenue portal</h1>
          <p className="mt-2 text-sm text-ink/60">
            Merchant uplift, fulfillment activity, and agent audit visibility.
          </p>
        </div>
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          <section className="border border-ink/10 bg-white p-5">
            <p className="text-sm text-ink/60">Merchant uplift</p>
            <p className="mt-3 text-3xl font-semibold">{money(uplift)}</p>
          </section>
          <section className="border border-ink/10 bg-white p-5">
          ...
          <section className="border border-ink/10 bg-white p-5">
            <p className="text-sm text-ink/60">Audit events</p>
        ...
        <section className="mt-8 border border-ink/10 bg-white p-6">
          ...
          <div className="mt-4 divide-y divide-ink/10">
            {revenue.length === 0 ? (
              ...
              revenue.slice(0, 20).map((row) => (
                <div
                  key={row.order_id}
                  className="flex items-center justify-between gap-4 py-3 text-sm"
                >
                  <span>Order {row.order_id.slice(0, 8)}</span>
