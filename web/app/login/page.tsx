  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-paper px-6">
      <section className="w-full max-w-md border border-ink/10 bg-white p-8 shadow-sm">
        ...
        <p className="mt-2 text-sm text-ink/60">Google creates or restores your Supabase-backed wallet account.</p>
        ...
        <form action={signInWithGoogle} className="mt-8">
          <button className="w-full bg-ink px-4 py-3 text-sm font-semibold text-paper" type="submit">Continue with Google</button>
        </form>
        <p className="mt-4 text-xs text-ink/50">Google OAuth must be enabled in Supabase Auth providers.</p>
      </section>
    </main>
  );
}
