...
// RecordReversal stores the gateway records a cancellation produced on the order
...
func (s *Store) RecordReversal(ctx context.Context, accountID, orderID string, result ReverseResult) error {
	...
	// The whole credited amount is always returned, inside the allowance, so there
	// is no partial reversal to report here any more.
	reason := "recorded at the payment gateway, returned to the wallet allowance"
